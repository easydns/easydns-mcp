...
import { dnsRecordsParsedSchema } from "../schemas.js";

export const definition = {
  name: "dns_records_parsed",
  description:
    "List DNS zone records in parsed zonefile format (human-readable).",
  inputSchema: {
...
export async function handler(
  ...
  const { domain } = dnsRecordsParsedSchema.parse(args);
  checkDomainAllowed(domain, config);
  const cid = createCorrelationId();

  const res = await client.request({
    method: "GET",
    path: `/zones/records/parsed/${encodeURIComponent(domain)}`,
  });

  audit({
    timestamp: new Date().toISOString(),
    correlationId: cid,
    tool: "dns_records_parsed",
    domain,
